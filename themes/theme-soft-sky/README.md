...
Install the ZIP with ETHOS Suite's local ZIP installer. For manual ZIP installation, create `scripts/theme-soft-sky/` on the transmitter and extract the ZIP contents into that folder.
...
To install from repository sources, copy `theme-soft-sky` into the transmitter `scripts` folder, restart, and select **Soft Sky**.
