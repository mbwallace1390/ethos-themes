...
Install the ZIP with ETHOS Suite's local ZIP installer. For manual ZIP installation, create `scripts/theme-rf-gold-pro/` on the transmitter and extract the ZIP contents into that folder.

A lightweight FrSky ETHOS theme based on the proven RF Blue Pro design.

- Bright gold outline focus
- Dark square controls
- Clear active, inactive, and disabled states
- Responsive 784x50 X20 / 464x50 X18 toolbar
- ETHOS-safe internal key `RFGold`
...
To install from repository sources, copy the complete `theme-rf-gold-pro` folder into the transmitter's `scripts` folder, restart, and select **RF Gold Pro** under **System > General > Theme**.
