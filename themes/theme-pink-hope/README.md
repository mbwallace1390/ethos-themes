...
Install the ZIP with ETHOS Suite's local ZIP installer. For manual ZIP installation, create `scripts/theme-pink-hope/` on the transmitter and extract the ZIP contents into that folder.
...
To install from repository sources, copy `theme-pink-hope` into the transmitter's `scripts` folder, restart, and select **Pink Hope** under **System > General > Theme**.
