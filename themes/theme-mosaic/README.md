...
Install the ZIP with ETHOS Suite's local ZIP installer. For manual ZIP installation, create `scripts/theme-mosaic/` on the transmitter and extract the ZIP contents into that folder.
...
To install from repository sources, copy `theme-mosaic` into the transmitter `scripts` folder, restart, and select **Mosaic** under **System > General > Theme**.
