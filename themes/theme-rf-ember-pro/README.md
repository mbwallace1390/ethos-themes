...
Install the ZIP with ETHOS Suite's local ZIP installer. For manual ZIP installation, create `scripts/theme-rf-ember-pro/` on the transmitter and extract the ZIP contents into that folder.
...
To install from repository sources, copy the complete `theme-rf-ember-pro` folder into the transmitter's `scripts` folder, restart, and select **RF Ember Pro** under **System > General > Theme**.
