...
Install the ZIP with ETHOS Suite's local ZIP installer. For manual ZIP installation, create `scripts/theme-deep-space/` on the transmitter and extract the ZIP contents into that folder.
...
To install from repository sources, copy `theme-deep-space` into the transmitter `scripts` folder, restart, then select **Deep Space** under **System > General > Theme**.
